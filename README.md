...
- Статистику слов-паразитов
- Информацию о паузах (количество, длительность)
- Структуру фраз
- Общие рекомендации
- Полный транскрипт

### Детализированный анализ с таймингами (рекомендуется для глубокого анализа)
...
- **Акценты и эмоциональные моменты** (типы: duration, repetition, content, pause)
- **Проблемные моменты** (типы: hesitation, long_pause, excessive_filler, fast_speech, slow_speech, incoherence)
- Данные для визуализации активности речи
- Рекомендации по конкретным временным отрезкам
...
- **Акценты и эмоциональные моменты** – различные типы акцентов:
  - `duration` – акцент через длительное произнесение слова
  - `repetition` – акцент через повторение слова
  - `content` – акцент через эмоциональные/усиленные слова
  - `pause` – акцент через паузу перед словом
- **Проблемные моменты** – выделенные проблемные участки речи:
  - `hesitation` – моменты колебания/нерешительности
  - `long_pause` – чрезмерно длинные паузы
  - `excessive_filler` – скопления слов-паразитов
  - `fast_speech` – слишком быстрая речь
  - `slow_speech` – слишком медленная речь
  - `incoherence` – сложные/запутанные фразы
